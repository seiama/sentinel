...
public final class Manual implements ImporterPunishmentSource {
  ...
  public ImporterPunishment asPunishment() {
    ...
    punishment.values.punisherId = this.punisher_id;
    ...
    punishment.values.staleAutomatic = this.stale_automatic;
    punishment.values.staleAt = this.stale_at;
    punishment.values.staleById = this.stale_by_id;
    punishment.values.staleByUsername = this.stale_by_username;
    punishment.values.staleByDiscriminator = this.stale_by_discriminator;
    punishment.values.staleReason = this.stale_reason;
    punishment.values.importBy = this.import_by != null ? this.import_by : ImporterConstants.IMPORTER_MANUAL;
    punishment.values.importId = this.import_id;
    punishment.values.importAt = this.import_at != null ? this.import_at : Instant.now();
    return punishment;
  }
}

...
public final class KickCommand implements GuildCommand {
  ...
  @Autowired
  private KickCommand(final PunishmentRepository punishments) {
  ...
  public @NotNull ApplicationCommandRequest request() {
    return ApplicationCommandRequest.builder()
      ...
      .addOption(Options.option(option -> option.name(Options.MEMBER).description("The member to kick").type(ApplicationCommandOption.Type.USER.getValue()).required(true)))
      .addOption(Options.option(option -> option.name(Options.REASON).description("The reason for kicking the member").type(ApplicationCommandOption.Type.STRING.getValue()).required(false)))
      .build();
  }

...
class MessageLinkUpdateHandler implements Function<MessageUpdateEvent, Publisher<Object>> {
  ...
  MessageLinkUpdateHandler(final GuildRepository guilds, final TemporaryMessageLinkRepository messageLinks, final GatewayDiscordClient relayRest) {
    this.guilds = guilds;
        ...
        return event.getGuild()
          ...
          .flatMap(TupleUtils.function((guild, message) -> {
            return this.guilds.findById(targetIds.guild())
              .flatMap(guildModel -> {
                return this.relayRest.getMessageById(targetIds.targetChannelId(), targetIds.targetMessageId())
                  .flatMap(msg -> msg.edit(
                    MessageEditSpec.builder()
                      .embeds(Possible.of(Optional.of(List.of(Appeals.createMessage(message.getData(), message.getAuthor(), Tuples.of(guild, guildModel))))))
                      .build()
                  ));
              });
          }));

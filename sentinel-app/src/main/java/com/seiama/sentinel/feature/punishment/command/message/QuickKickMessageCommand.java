...
public final class QuickKickMessageCommand implements MessageCommand {
  ...
  @Autowired
  private QuickKickMessageCommand(final Punishments punishments) {
  ...
  public Mono<?> on(final GatewayDiscordClient client, final MessageInteractionEvent event, final Guild guild) {
    ...
    return this.punishments.createUsing(new MessageInteractionPunishmentCreator(client, event, event.getResolvedMessage(), guild, PunishmentModel.Type.KICK, PunishmentAction.kick(), reason));
  }
}
